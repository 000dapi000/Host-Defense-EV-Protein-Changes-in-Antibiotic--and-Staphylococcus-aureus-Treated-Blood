...
# ---- [Load Bac Infection Group Expression Data] ----
data_bac <- utils::read.table(
"normalized_bac_infection_group_P257_09.txt",
header = TRUE,
sep = "\t",
quote = "",
check.names = FALSE
...
colnames(Annotation_Human_SA) <- base::gsub("[[:punct:]]", "_", colnames(Annotation_Human_SA))
base::stopifnot(base::all(c("Entry", "Gene_Names__primary_") %in% colnames(Annotation_Human_SA)))

# ---- [Clean Gene Names: fallback to Protein_IDs] ----
data_main$Gene_names <- base::ifelse(
base::is.na(data_main$Gene_names) | base::trimws(data_main$Gene_names) == "",
data_main$Protein_IDs,
data_main$Gene_names
)
data_main$Gene_names <- base::make.unique(base::as.character(data_main$Gene_names))

data_bac$Gene_names <- base::ifelse(
base::is.na(data_bac$Gene_names) | base::trimws(data_bac$Gene_names) == "",
data_bac$Protein_IDs,
data_bac$Gene_names
)
data_bac$Gene_names <- base::make.unique(base::as.character(data_bac$Gene_names))

# ---- [Define Expression Groups for Main Data] ----
group_cols_main <- list(
HC = base::grep("^HC_[1-6]$", colnames(data_main), value = TRUE),
BCN = base::grep("^BCN_[1-6]$", colnames(data_main), value = TRUE),
BCP = base::grep("^BCP_[1-6]$", colnames(data_main), value = TRUE)
)

# ---- [Define Expression Groups for Bac Infection Data] ----
group_cols_bac <- list(
HC = base::grep("^HC_[1-6]$", colnames(data_bac), value = TRUE),
Bac_infection_group = base::grep("^Bac_infection_group_[0-9]+$", colnames(data_bac), value = TRUE)
)
...
label_up <- base::paste("Higher in", group1)
label_down <- base::paste("Higher in", group2)

# ---- [Choose Dataset and Group Definitions] ----
if (comp_name == "Bac_infection_group_vs_HC") {
data_used <- data_bac
group_cols_used <- group_cols_bac
} else {
data_used <- data_main
group_cols_used <- group_cols_main
}

g1_cols <- group_cols_used[[group1]]
g2_cols <- group_cols_used[[group2]]

base::stopifnot(base::length(g1_cols) > 0, base::length(g2_cols) > 0)

base::dir.create(comp_name, showWarnings = FALSE)

# ---- [Create Data Frame with Expression & IDs] ----
df <- base::data.frame(
Gene = data_used$Gene_names,
Protein_IDs = data_used$Protein_IDs,
data_used[, c(g1_cols, g2_cols)],
...
vals1 <- base::as.numeric(df[i, g1_cols])
vals2 <- base::as.numeric(df[i, g2_cols])

if (base::all(base::is.na(vals1)) || base::all(base::is.na(vals2))) next

vals1 <- vals1[!base::is.na(vals1)]
vals2 <- vals2[!base::is.na(vals2)]

if (base::length(vals1) < 2 || base::length(vals2) < 2) next

# ---- [Welch t-test] ----
test_result <- base::tryCatch(
stats::t.test(vals1, vals2, paired = FALSE, var.equal = FALSE),
error = function(e) NULL
)

if (!base::is.null(test_result)) {
df$log10FC[i] <- base::mean(vals1, na.rm = TRUE) - base::mean(vals2, na.rm = TRUE)
...
df$negLog10P <- -base::log10(df$adj_p_value)

# ---- [Annotation] ----
df <- dplyr::mutate(df, First_ID = base::sub(";.*", "", Protein_IDs))

Annotation_subset <- Annotation_Human_SA[, c("Entry", "Gene_Names__primary_"), drop = FALSE]
colnames(Annotation_subset) <- c("First_ID", "Annotated_Gene")

df <- dplyr::left_join(df, Annotation_subset, by = "First_ID")

df$Annotated_Gene[
base::is.na(df$Annotated_Gene) | df$Annotated_Gene == ""
] <- df$First_ID[
base::is.na(df$Annotated_Gene) | df$Annotated_Gene == ""
]
...
df$log10FC > fc_threshold & df$negLog10P > -base::log10(pval_threshold) ~ label_up,
df$log10FC < -fc_threshold & df$negLog10P > -base::log10(pval_threshold) ~ label_down,
TRUE ~ "Non-significant"
)

df$group <- base::factor(df$group, levels = c(label_up, label_down, "Non-significant"))

# ---- [Export Result Tables] ----
utils::write.table(
df,
file = base::file.path(comp_name, base::paste0("All_proteins_", comp_name, ".txt")),
sep = "\t",
row.names = FALSE,
quote = FALSE
)
...
utils::write.table(
df[df$group == label_up, , drop = FALSE],
file = base::file.path(comp_name, base::paste0("Upregulated_in_", group1, "_", comp_name, ".txt")),
sep = "\t",
row.names = FALSE,
quote = FALSE
)
...
utils::write.table(
df[df$group == label_down, , drop = FALSE],
file = base::file.path(comp_name, base::paste0("Upregulated_in_", group2, "_", comp_name, ".txt")),
sep = "\t",
row.names = FALSE,
quote = FALSE
)
...
# ---- [Top Labels] ----
top_up <- dplyr::filter(df, group == label_up, Annotated_Gene != "")
top_up <- dplyr::arrange(top_up, dplyr::desc(negLog10P), dplyr::desc(base::abs(log10FC)))
top_up <- utils::head(top_up, 10)

top_down <- dplyr::filter(df, group == label_down, Annotated_Gene != "")
top_down <- dplyr::arrange(top_down, dplyr::desc(negLog10P), dplyr::desc(base::abs(log10FC)))
top_down <- utils::head(top_down, 10)

# ---- [Color Map for Plot] ----
color_map <- c(
stats::setNames("red", label_up),
stats::setNames("blue", label_down),
...
df,
ggplot2::aes(x = log10FC, y = negLog10P, color = group)
) +
ggplot2::geom_point(
ggplot2::aes(alpha = group),
size = 3.5
) +
ggplot2::scale_color_manual(
values = color_map,
breaks = c(label_up, label_down, "Non-significant")
) +
ggplot2::scale_alpha_manual(
values = c(
stats::setNames(0.8, label_up),
stats::setNames(0.8, label_down),
"Non-significant" = 0.4
),
guide = "none"
) +
ggplot2::geom_hline(
yintercept = -base::log10(pval_threshold),
linetype = "dashed",
color = "black",
linewidth = 0.6
) +
...
if (base::nrow(top_up) > 0) {
volcano_plot <- volcano_plot +
ggrepel::geom_text_repel(
data = top_up,
mapping = ggplot2::aes(x = log10FC, y = negLog10P, label = Annotated_Gene),
color = "red",
size = 10,
max.overlaps = 100,
box.padding = 0.5,
...
ggplot2::theme(
plot.title = ggplot2::element_text(face = "bold", hjust = 0.5, size = 32),
axis.title = ggplot2::element_text(size = 32, face = "bold"),
...
ggplot2::guides(color = ggplot2::guide_legend(override.aes = list(size = 8, alpha = 1)))

# ---- [Save Plot: Large canvas for big text] ----
ggplot2::ggsave(
filename = base::file.path(comp_name, base::paste0("Volcano_", comp_name, ".png")),
plot = volcano_plot,
dpi = 300,
width = 15,
height = 12,
...
base::cat("✅ Volcano plot saved for", comp_name, "\n")
}

base::cat("🎉 All volcano plots complete with full legend including Non-significant!\n")
...
mutate(
Pathway = paste0(Pathway, " [", gsub("_Human", "", source), "]"),
log10_p = -log10(p_adj),
Source = source
) %>%
...
cat("\n🎉 All enrichment heatmaps generated WITHOUT titles and clean layout!\n")
